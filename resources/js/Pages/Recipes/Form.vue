    ...
    <Layout>
        <div class="py-10 min-h-screen relative mx-auto max-w-5xl px-4 pb-12 sm:px-6 lg:px-8">
            ...
            <div class="mx-auto mb-10 text-center">
                <h1 class="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white tracking-tight drop-shadow-sm">
                    Share Your <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-600">Masterpiece</span>
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-2 text-base sm:text-lg">Step {{ currentStep }} of {{ totalSteps }}: {{
            ...
            <form @submit.prevent="submit" class="relative mx-auto max-w-3xl">
                <transition name="slide-fade" mode="out-in">
                    ...
                    <div v-if="currentStep === 1" key="step1"
                        class="space-y-8 p-6 sm:p-8 md:p-10 rounded-[2rem] sm:rounded-[2.5rem] bg-white/70 dark:bg-gray-900/70 border border-white/60 dark:border-gray-800 shadow-xl backdrop-blur-2xl">
                        <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-2 gap-4 sm:gap-6">
                            <div class="space-y-2">
                                ...
                                <input v-model="form.cook_time" type="number"
                                    class="glass-input w-full text-center font-black text-2xl sm:text-3xl h-20 sm:h-24"
                    ...
                    <div v-else-if="currentStep === 3" key="step3"
                        class="space-y-6 p-6 sm:p-8 md:p-10 rounded-[2rem] sm:rounded-[2.5rem] bg-white/70 dark:bg-gray-900/70 border border-white/60 dark:border-gray-800 shadow-xl backdrop-blur-2xl min-h-[400px]">
                        <div class="flex items-center justify-between px-1 mb-4">
                            <label class="font-bold text-xl sm:text-2xl text-gray-800 dark:text-white">Ingredients</label>
                            <button type="button" @click="addIngredient"
                                class="group flex items-center gap-2 rounded-full bg-orange-50 dark:bg-orange-950/50 px-4 sm:px-5 py-2.5 text-sm font-bold text-orange-600 dark:text-orange-400 transition-all hover:bg-orange-100 dark:hover:bg-orange-900/50 hover:scale-105 active:scale-95 shadow-sm">
                                <span>+ Add Item</span>
                            </button>
                        </div>
                            ...
                            <transition-group name="list">
                                <div v-for="(ingredient, index) in form.ingredients" :key="index"
                                    ...
                                    <button type="button" @click="removeIngredient(index)"
                                        class="h-10 w-10 flex-shrink-0 flex items-center justify-center rounded-full bg-red-50 dark:bg-red-950/50 text-red-400 opacity-50 hover:opacity-100 hover:bg-red-100 dark:hover:bg-red-900/50 hover:text-red-600 transition-all active:scale-90">
                                        &times;
                                    </button>
                                </div>
                            </transition-group>
                    ...
                    <div v-else key="step4"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative group cursor-pointer">
                                ...
                                <div class="border-3 border-dashed border-gray-300/60 dark:border-gray-600/60 rounded-3xl p-8 sm:p-10 text-center bg-white/40 dark:bg-white/5 transition-all group-hover:bg-orange-50/50 dark:group-hover:bg-orange-950/20 group-hover:border-orange-300 dark:group-hover:border-orange-700 group-hover:scale-[1.01]"
                                    :class="{ 'border-red-400 bg-red-50 dark:bg-red-950/20': form.errors.images }">
                                    <div class="flex flex-col items-center gap-3">
                                        <div
                                            class="h-16 w-16 rounded-full bg-orange-100 dark:bg-orange-950/50 text-orange-600 dark:text-orange-400 flex items-center justify-center mb-2 shadow-sm">
                ...
                <div class="mt-8 flex items-center justify-between">
                    <button type="button" @click="prevStep"
                        class="px-5 sm:px-6 py-3 rounded-xl font-bold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white transition-colors"
                        :class="{ 'invisible': currentStep === 1 }">
                        &larr; Back
                    </button>

                    <button v-if="currentStep < totalSteps" type="button" @click="nextStep"
                        class="px-6 sm:px-8 py-3 rounded-xl bg-gray-900 dark:bg-white/10 font-bold text-white shadow-lg hover:bg-black dark:hover:bg-white/20 hover:shadow-xl hover:-translate-y-1 transition-all">
                        Next Step &rarr;
                    </button>

                    <button v-else :disabled="form.processing"
                        class="px-8 sm:px-10 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-red-600 font-bold text-white shadow-lg shadow-orange-500/30 hover:shadow-orange-500/50 hover:scale-105 transition-all disabled:opacity-70 disabled:scale-100">

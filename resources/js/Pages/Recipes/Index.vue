...
// Native debounce (No lodash required)
...
// Automatically watch for changes and apply filters with a 300ms debounce
watch(
...
const clearFilters = () => {
    // Resetting these triggers the watcher above automatically
    search.value = "";
    selectedCategory.value = "";
    selectedDifficulty.value = "";
    maxTime.value = "";
    ...
    <Layout>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-12">
            ...
            <section class="text-center space-y-4 pt-4">
                <h1 class="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight text-gray-900">
                    Explore the
                    <span
            ...
            <section class="relative z-20">
                <div
                    class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100/80 p-6 sm:p-8 transition-all duration-300">
                    ...
                    <div class="relative flex items-center w-full mb-6">
                        ...
                        <input v-model="search" type="text" placeholder="Search by recipe, ingredient, or chef..."
                            class="block w-full pl-14 pr-4 py-4 bg-gray-50/50 border border-gray-200 rounded-2xl text-gray-900 focus:bg-white focus:ring-4 focus:ring-orange-500/15 focus:border-orange-500 transition-all duration-300 text-lg placeholder:text-gray-400 placeholder:font-medium" />
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        ...
                        <div class="relative group">
                            <label
                                class="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 pl-1">Category</label>
                            <div class="relative">
                                <select v-model="selectedCategory"
                                    class="block w-full appearance-none bg-gray-50/50 border border-gray-200 text-gray-700 py-3.5 px-4 pr-10 rounded-xl focus:bg-white focus:outline-none focus:ring-4 focus:ring-orange-500/15 focus:border-orange-500 transition-all font-medium cursor-pointer">
                        ...
                        <div class="relative group">
                            <label
                                class="block text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 pl-1">Difficulty</label>
                            <div class="relative">
                        ...
                        <div class="relative group">
                            <label
                                class="flex justify-between items-end text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2 pl-1 pr-1">
                                <span>Prep Time</span>
                            ...
                            <div class="relative flex items-center">
                                ...
                                <input v-model="maxTime" type="number" min="1" placeholder="Max minutes (e.g. 30)"
                                    class="block w-full pl-11 pr-4 py-3.5 bg-gray-50/50 border border-gray-200 rounded-xl text-gray-900 focus:bg-white focus:outline-none focus:ring-4 focus:ring-orange-500/15 focus:border-orange-500 transition-all font-medium placeholder:text-gray-400" />
                            </div>
                        </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-2.5 text-sm font-medium text-gray-500">
                            <div class="relative flex h-2.5 w-2.5">
                                ...
                                <span class="relative inline-flex rounded-full h-2.5 w-2.5"
                                    :class="hasActiveFilters ? 'bg-orange-500' : 'bg-gray-300'"></span>
                            </div>
                            <span v-if="hasActiveFilters" class="text-gray-700">Filtering results dynamically...</span>
                        ...
                        <transition enter-active-class="transition ease-out duration-200"
                            ...
                            <button v-if="hasActiveFilters" @click="clearFilters" type="button"
                                class="text-sm font-bold text-gray-500 hover:text-red-600 transition-colors flex items-center gap-1.5 py-1 px-2 rounded-lg hover:bg-red-50">
            ...
            <section class="relative z-10 pt-4">
                ...
                <div v-if="visibleRecipes.length === 0"
                    class="flex flex-col items-center justify-center py-24 rounded-[2rem] border-2 border-dashed border-gray-200 bg-white/50 backdrop-blur-sm">
                    <div
                        class="mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-orange-50 text-orange-500 shadow-inner">
                    ...
                    <h2 class="text-xl sm:text-2xl font-bold text-gray-900 mb-2">No recipes found</h2>
                    <p class="text-base text-gray-500 max-w-md text-center">
                    ...
                    <Link v-for="(recipe, index) in visibleRecipes" :key="recipe.id" :href="`/recipes/${recipe.id}`"
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-8 sm:p-10">
                            <div class="transform transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                                ...
                                <h3
                                    class="text-3xl sm:text-4xl font-black text-white mb-6 leading-tight drop-shadow-md line-clamp-2">
                                    {{ recipe.title }}
                                </h3>
                                ...
                                <div class="flex items-center justify-between border-t border-white/20 pt-6">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="h-10 w-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center text-white border border-white/30 font-bold text-lg">
                                        ...
                                        <div class="flex flex-col text-white">
                                            <span class="text-xs font-medium opacity-80 uppercase tracking-wide">Recipe
                                                By</span>
                                            <span class="text-sm font-bold">{{ recipe.author_name || 'Unknown Chef'
                                            }}</span>
                                        </div>
                                    </div>
...
/* Fallback for input[type="number"] arrows */
